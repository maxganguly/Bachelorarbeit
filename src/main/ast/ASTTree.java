...
public class ASTTree {
	...
	public final static Set<String> REMOVE_FROM_GENERALIZE = 
			Set.of(new String[] {"unary", "var", "assign", "binary"});
	...
	public ASTTree(String source, ASTTree parent) {
		...
		this.name = name;
		this.type = type;
		System.out.println(tag+":"+name+":"+type);
	...
	public boolean equals(ASTTree other) {
		if (	strEqual(this.tag,other.tag) && 
				strEqual(this.name,other.name) && 
				strEqual(this.type,other.type) &&
	...
	public boolean containsExact(ASTTree search) {
		...
		if (strEqual(this.tag,search.tag) && 
				strEqual(this.name,search.name) && 
				strEqual(this.type,search.type)) {
			//if root type is accepted check if any child has all children of the searched tree
			for (ASTTree a1 :this.children) {
